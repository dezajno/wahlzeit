...
public class PhotoFilter implements Serializable {
	...
	protected List<PhotoId> getFilteredPhotoIds() {
		...
		if (noFilterConditions == 0) {
			candidates = PhotoGlobals.getInstance().getManager().getPhotoCache().keySet();
		} else {
			List<Tag> tags = new LinkedList<Tag>();
			candidates = new LinkedList<PhotoId>();
			for (String condition : getFilterConditions()) {
				PhotoGlobals.getInstance().getManager().addTagsThatMatchCondition(tags, condition);
			}
		...
		for (PhotoId candidateId : candidates) {
			Photo photoCandidate = PhotoGlobals.getInstance().getManager().getPhoto(candidateId);
